...
class TestUserContext(UserContext):
    ...
    __test__ = False
    default_auth_provider: ClassVar[AuthProvider | None] = None

    def __init__(self):
        if TestUserContext.default_auth_provider is None:
            TestUserContext.default_auth_provider = AuthProvider.from_env(config=Config.from_env())
        self.access_token = TestUserContext.default_auth_provider.get_access_token()
